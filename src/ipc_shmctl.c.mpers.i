        ...
        const unsigned int cmd)
{
 ...
 if (umoven_or_printaddr((tcp), (addr), sizeof(*(&info)), (void *) (&info)))
  return;
 tprint_struct_begin();
 do { tprints_field_name("used_ids"); tprintf_string("%lld", (sizeof((info).used_ids) == sizeof(char) ? (long long) (char) ((info).used_ids) : sizeof((info).used_ids) == sizeof(short) ? (long long) (short) ((info).used_ids) : sizeof((info).used_ids) == sizeof(int) ? (long long) (int) ((info).used_ids) : sizeof((info).used_ids) == sizeof(long) ? (long long) (long) ((info).used_ids) : (long long) ((info).used_ids))); } while (0);
 tprint_struct_next();
